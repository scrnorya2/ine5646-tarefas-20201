...
export function podeAdicionarUsuario(login: Login): {login: Login} | null {
  const soPalavras = /\W/
  
  return login.trim().length > 10 || 
    login.trim().length === 0 || 
    soPalavras.test(login.trim()) || 
    usuarios.has(login) ? null : {login: login.trim()}
}
