...
export async function removeUsuarioDoBanco(broker: ServiceBroker, token: TokenJWT): Promise<RespostaDeuNaoDeu> {
  try {
...
export async function cadastra(broker: ServiceBroker, origin: string, nomeApp: string,
  ...
  try {  
    ...
    const tokenConfirmacao: TokenUUID = uuid()
...
export async function renovaToken(token: TokenJWT): Promise<RespostaRenovaToken> {
  try {
    ...
    if (dados == null)
      ...
      return jwt.sign(dadosToken, process.env.JWT_PASSWORD, {expiresIn: process.env.JWT_DURATION})
    }
  } catch (erro) {
    return null
  }
...
export function decodificaJWT(token: TokenJWT): TokenJWTDecodificado | null {
  try {
    const dados: TokenJWTDecodificado = jwt.verify(token, process.env.JWT_PASSWORD)
    return dados
  } catch (e) {
    return null
  }
